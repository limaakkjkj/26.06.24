...
const imagens = [
  ...
  const tamanhoImagem = 100;
  
  function criarImagem(src, index) {
    const imgElemento = document.createElement('img');
    imgElemento.src = src;
    imgElemento.style.position = "absolute"; 
    if (index === 0) { 
      imgElemento.style.top = 0;
      imgElemento.style.left = 0;
      imgElemento.style.width = larguraTotalCenario + medida;
      imgElemento.style.height = alturaTotalCenario + medida;
    } else { 
      imgElemento.style.top = `${Math.random() * (alturaTotalCenario - 100)}px`; 
      imgElemento.style.left = `${Math.random() * (larguraTotalCenario - 100)}px`; 
      imgElemento.style.width = `${tamanhoImagem}px`;
        imgElemento.style.height = `${tamanhoImagem}px`; 
    }
    imgElemento.style.zIndex = -1;
    return imgElemento;
  }
  
  imagens.forEach((src, index) => {
    const imgElemento = criarImagem(src, index);
    divCenario.appendChild(imgElemento);
  });
